...
def circles():
    ...
    for j in range(drone_count):
        i = assignments[j]
        if(j < 4):
            drone_step_sequence[i, :, start_circles_timestep:] = layer1_circling_paths[j][:, 0:length]
        else:
            drone_step_sequence[i, :, start_circles_timestep:] = layer2_circling_paths[j - 4][:, 0:length]
